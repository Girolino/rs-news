...
- `git checkout` e comandos destrutivos (`rm -rf`, semelhantes) exigem autorização explícita do usuário antes de serem executados.
...
6. **Documentation Loop & Workflows**: if o trabalho gerou novo conhecimento ou alterou comportamento:
   - Atualize o guia relevante em `docs/llm-guide/` (ou crie um novo).
   - Se houver alterações em fluxos Mermaid (`__workflows__/*.mmd`), valide-os com `@mermaid-js/mermaid-cli` e confira a renderização em `/workflows` antes de finalizar.
